...
A customer building a combo via the generic Build a Birthday flow (not a dedicated package page) sees every savings source called out, not just the monetary discount — if they've also crossed a free-addon threshold (E-Invite, Tattoo Artist, or the ₹5,000 activity credit — see the tiers table above), that saving is shown too, not folded quietly into a lower total. `trueMRP()` in builder.html sums every cart item's real pre-savings value (its normal price, or its original price for anything zeroed/discounted by a freebie threshold) so the bottom bar, mini-cart, checkout Order Summary, and the Review page (as of 2026-08-24 — it previously used a different, percentage-only `dp()` calc that undercounted whenever a freebie was involved) can all show one clearly-labelled figure with **all** savings combined — the monetary discount and every unlocked free addon — instead of only part of the story. (The free-pinata threshold this originally covered was removed 2026-08-18 — see Pinata below — so today the freebie thresholds feeding into this are E-Invite, Tattoo Artist, and the ₹5,000 activity credit, all added 2026-08-24.)

### Where the freebie/discount badge row shows — and where it doesn't
_Added 2026-08-24, per Shruti — "Don't show the discount/freebie bar at checkout page. Add the freebies and discounts in the order summary. The same should be visible on the admin page and the emails."_

The `.bb-freebies` badge row (see `updFreebieWidget()`) is a nudge to add *more* to the cart, so it only makes sense while the cart is still being built — it's hidden on the checkout screen (`'sco'`). At checkout, the same information (which freebies unlocked + the ₹ total saved) instead appears as line items in the Order Summary box (`updCoAmt()`, `#coFreebiesRow`). That same breakdown is also sent to the backend on booking submit (`order_total_savings`, `order_freebies_text` on the `/api/leads/submit` payload — see `LeadSubmitRequest` in `routers/leads.py`) so it shows up identically on admin.html (Billing & Rewards → Total Savings / Freebies Unlocked) and in both the customer confirmation and internal team emails. **Requires `migrations/019_add_order_savings_freebies_to_leads.sql` to be run against the production DB before/with this backend deploy** — the INSERT will fail on every booking submission until those two columns exist.
