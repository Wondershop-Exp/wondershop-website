...
- [ ] **Tune the "What does the birthday child like most?" recommendation matching** - added 2026-08-16 (Step 0 multiselect → recommends decor/activities/return gifts/invite designs, with a "Show All" fallback everywhere). Matching is a lightweight keyword heuristic (see `const INTERESTS` near the top of builder.html's script, each interest's `kw` list) rather than a hand-tagged mapping of every catalogue item — works well for decor/activities/invites (which are theme-named), weaker for return gifts (mostly generic products like bags/tumblers, not theme-tied, so only board games/sports items reliably match). Review the `kw` lists and adjust once real bookings show what customers pick.
- [ ] **Re-add "Share Booking Details on WhatsApp" on order confirmation** - removed 2026-08-16 per Shruti ("we'll modify it and add it later"); the `shareWA()` function is still in builder.html, just unused — bring back the button once the new share message/flow is ready.
